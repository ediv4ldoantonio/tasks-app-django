...
class TestUser:
    ...
    def test_non_admin_retrieve_data(self, api_client, user_factory, authenticate_user):
    ...
    def test_deny_delete_to_non_admin(self, api_client, user_factory, authenticate_user):
        app_user = user_factory(firstname="First")
        user = authenticate_user(is_admin=False)
        token = user['token']
        api_client_with_credentials(token, api_client)
